...
class InboxServer(smtpd.SMTPServer, object):
    ...
    def process_message(self, peer, mailfrom, rcpttos, data):
        ...
        subject = self.parse_subject(Parser().parsestr(data)['subject']).encode('raw_unicode_escape').decode('utf-8')
        sender = parseaddr(Parser().parsestr(data).get('From'))[1].encode('raw_unicode_escape').decode('utf-8')
        sendername = parseaddr(Parser().parsestr(data).get('From'))[0].encode('raw_unicode_escape').decode('utf-8')
        sentto = parseaddr(Parser().parsestr(data).get('To'))[1].encode('raw_unicode_escape').decode('utf-8')
        senttoname = parseaddr(Parser().parsestr(data).get('To'))[0].encode('raw_unicode_escape').decode('utf-8')
        mailplain = None
        mailhtml = None
        attachments = []
        for part_of_mail in Parser().parsestr(data).walk():
            ...
            for mailpart in mailcontent:
                if not (mailpart[0] == "text/html" or mailpart[0] == "text/plain"):
                    ...
                    mailhtml = mailpart[1].encode('raw_unicode_escape').decode('utf-8')
                elif mailpart[0] == "text/plain":
                    mailplain = mailpart[1].encode('raw_unicode_escape').decode('utf-8')
